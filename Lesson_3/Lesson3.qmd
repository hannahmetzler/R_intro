...
# Comment on the lesson plan: Sections up to 6 Figures script done in lesson 3, 7-10 done in lesson4 in the skills training 2025. 
...
1.  On [https://github.com]():
    1.  Make a remote git repository (e.g. “rintro_lesson3”)
    2.  Copy the SSH link.
2.  In Rstudio: Clone the remote repository via the Terminal (quicker) or the Rstudio interface:
    1.  Terminal: navigate to your folder for this R course using `cd`. Type `git clone` and paste your SSH link.
    2.  Rstudio interface: Create a new project & choose `Version Control`, then `Git`. Under `Clone Git Repository`, enter your SSH link.
3.  Then organize your repository:
    1.  In the now local folder of the repository, create sub folders (e.g. “data”, “figures”, “code”, “write_up”).
    2.  Put the [data file](https://github.com/hannahmetzler/R_intro_lesson3/blob/main/data/lesson2_data_babynames.txt) for this lesson in your “data” folder.
    3.  Make a new R Project based in your main directory folder (e.g. “rintro_lesson3”).
...
-   Next, we read in data, with the same function as last time but for .txt files. You'll see a message because we did not specify the variable type for the columns. 
...
head(data)
```
- Use `summary(data`) and `glimpse(data)` to see how the output changes before and after changing the column type. 
...
- To change a columns' variable type, you can use the argument `col_types` when reading in the data (see ?read_tsv): 
...
-   Check if your own name exists in the data set using `filter()` from dplyr:
    -   Check the help entry: `?dplyr::filter` (Description, Usage & Examples).
...
- Check the filtered data frame, for example using `head()`. 
...
- To make our code easier to read and add to, we're going to use something called a pipe (%>%).  Imagine it like a series of steps: the pipe helps you move from one step to the next, passing the data along as you go. 
  - In the first step, `filter()` will be run on `data` before assigning the output to `data_clean`.
    -   Shortcut for %>%: `Shift + Ctrl/Cmd + M`
...
To run a linear regression, our dependent variable needs to be normally distributed. A perfect normal distribution is symmetrical, for example, here are 2 distributions with n = 1 million observations:

```{r, histogram nv,  echo=F, fig.height=3.5}
...
```{r, histogram names, fig.asp=1, fig.width=3, warning=T}
## MAKE FIGURES ####
...
-   You might want more explicit labels for "F" and "M" in a figure. We can update the code to create the `data_figs` data frame at the top of our script.
-   We'll use `mutate()` to change the names of the variable levels of the variable sex.
-   We'll use `factor()`, a function to make categorical variables with labels. The arguments we need are (from the help section):
...
End of lesson 3 LBG Career Center Skills Training.
...
-   In green, you see the residual for the data point: The difference between the model prediction and the actual data point.
...
-   The slope b is 19.97 g: the mean weight difference between the Diets. On average, Diet 2 chicks are this much heavier.
...
-   Delete everything below the chunk at the top (the `YAML`) that starts and ends with `---`.
...
-   Add `echo = FALSE` (or only the F) in the curly brackets after r: \`\`\``{r, echo = FALSE}`. This let's R know you do not want to print out this code chunk in the report, but only its results.
...
- At the end, don't forget to commit and push to GitHub. 
